...
// Blog yazılarını çek
$sql = "SELECT b.id id, b.title, c.id city_id, c.name,b.created_at, b.updated_at FROM blog_posts b JOIN cities c ON b.city_id = c.id ORDER BY created_at DESC";
    ...
    <div class="content">
        ...
        <div class="container mt-4">
            <div class="d-flex justify-content-between align-items-center">
                <h2 class="fw-bold">Blog Yönetimi</h2>
                <a href="blog_ekle.php" class="btn btn-primary"><i class="fas fa-plus"></i> Yeni Blog Ekle</a>
            </div>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            ...
                            <tr>
                                <td><?= $row['id'] ?></td>
                                <td><a href="../blog.php?id=<?= $row['id'] ?>"><?= htmlspecialchars($row['title']) ?></a></td>
                                <td><?= $row['name'] ?></td>
                                <td><?= $row['created_at'] ?></td>
                                <td><?= $row['updated_at'] ?></td>
                                <td class="text-center">
                                    <a href="blog_duzenle.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-outline-primary">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="blog_sil.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-outline-danger"
