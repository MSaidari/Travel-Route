<?php
session_start();
$host = "localhost";
$dbname = "gezirotasi";
$username = "root";
$password = "12345678";

// Veritabanı bağlantısı
$conn = new mysqli($host, $username, $password, $dbname);
if ($conn->connect_error) {
    die("Veritabanı bağlantısı başarısız: " . $conn->connect_error);
}

// Ülkeleri getir
$sql = "SELECT * FROM countries ORDER BY name ASC";
$countries = $conn->query($sql);
?>

<!DOCTYPE html>
<html lang="tr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Şehir Yönetimi</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.2/css/all.min.css">
    <link rel="stylesheet" href="../../css/admin/admin_blogs.css">
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>

</head>
<body>
    <!-- Sidebar -->
    <div class="sidebar" id="sidebar-wrapper">
        <a href="#" class="sidebar-toggle" id="menu-toggle"><i class="fas fa-bars"></i></a>
        <a href="../admin.php"><i class="fas fa-home"></i> <span>Dashboard</span></a>
        <a href="admin_blogs.php"><i class="fas fa-newspaper"></i> <span>Blog Yönetimi</span></a>
        <a href="admin_places.php"><i class="fas fa-map-marked-alt"></i> <span>Mekan Yönetimi</span></a>
        <a href="admin_cities.php" class="active"><i class="fas fa-city"></i> <span>Şehir Yönetimi</span></a>
        <a href="admin_countries.php"><i class="fas fa-globe"></i> <span>Ülke Yönetimi</span></a>
        <a href="admin_reservations.php"><i class="fas fa-calendar-check"></i> <span>Rezervasyonlar</span></a>
        <a href="admin_reviews.php"><i class="fas fa-comments"></i> <span>Yorumlar</span></a>
        <a href="admin_tours.php"><i class="fas fa-route"></i> <span>Turlar</span></a>
        <a href="admin_iletisim.php"><i class="fas fa-envelope"></i> <span>İletişim</span></a>
    </div>

    <!-- İçerik -->
    <div class="content">
        <nav class="navbar">
            <span class="navbar-text">Administrator</span>
            <a href="../cikis.php" class="btn btn-outline-danger"><i class="fas fa-sign-out-alt"></i> Çıkış Yap</a>
        </nav>

        <div class="container mt-4">
            <div class="d-flex justify-content-between align-items-center">
                <h2 class="fw-bold">Şehir Yönetimi</h2>
                <a href="city_ekle.php" class="btn btn-primary"><i class="fas fa-plus"></i> Yeni Şehir Ekle</a>
            </div>
            <!-- Ülkeler Kart Şeklinde Listelenecek -->
            <div class="row mt-4">
                <?php while ($row = $countries->fetch_assoc()): ?>
                    <div class="col-md-4">
                        <div class="card country-card mb-3" data-country="<?= $row['id'] ?>">
                            <div class="card-body text-center">
                                <h5 class="card-title"><?= htmlspecialchars($row['name']) ?></h5>
                                <button class="btn btn-outline-primary view-cities" data-country="<?= $row['id'] ?>">
                                    <i class="fas fa-city"></i> Şehirleri Görüntüle
                                </button>
                            </div>
                        </div>
                    </div>
                <?php endwhile; ?>
            </div>

            <!-- Şehirleri Listeleyeceğimiz Alan -->
            <div id="city-container" class="mt-4"></div>
        </div>
    </div>

    <script>
        $(document).ready(function() {
            $(".view-cities").click(function() {
                let countryId = $(this).data("country");
                let cityContainer = $("#city-container");

                $.ajax({
                    url: "get_cities.php",
                    type: "POST",
                    data: { country_id: countryId },
                    success: function(response) {
                        if ($.trim(response) === "") {
                            cityContainer.html("<p class='text-center text-danger'>Bu ülkede kayıtlı şehir bulunamadı.</p>");
                        } else {
                            let cityTable = '<table class="table table-bordered">' +
                                '<thead class="table-light"><tr><th>Şehir</th><th>İşlemler</th></tr></thead><tbody>';

                            let cities = $(response).filter("option").slice(1);
                            cities.each(function() {
                                let cityId = $(this).val();
                                let cityName = $(this).text();
                                cityTable += '<tr>' +
                                    '<td>' + cityName + '</td>' +
                                    '<td class="text-center">' +
                                    '<a href="city_duzenle.php?id=' + cityId + '" class="btn btn-sm btn-outline-primary">' +
                                    '<i class="fas fa-edit"></i></a> ' +
                                    '<a href="city_sil.php?id=' + cityId + '" class="btn btn-sm btn-outline-danger" ' +
                                    'onclick="return confirm(\'Bu şehri silmek istiyor musunuz?\')">' +
                                    '<i class="fas fa-trash"></i></a>' +
                                    '</td></tr>';
                            });

                            cityTable += '</tbody></table>';
                            cityContainer.html(cityTable);
                        }
                    },
                    error: function(xhr, status, error) {
                        cityContainer.html("<p class='text-danger'>Şehirler yüklenirken hata oluştu: " + error + "</p>");
                    }
                });
            });
        });
        document.getElementById("menu-toggle").addEventListener("click", function () {
            document.getElementById("sidebar-wrapper").classList.toggle("collapsed");
        });
    </script>

</body>
</html>
